...
$$
\begin{aligned}
  \operatorname{dv}_{ij} &= \beta_{0i} + \beta_1(\operatorname{iv}_{\operatorname{T2}_j}) + \beta_2(\operatorname{iv}_{\operatorname{T3}_j}) + \varepsilon_{ij} \\
...
The residuals look reasonably normally distributed, and there seems to be fairly constant variance across the linear predictor
```{r}
plot(m1)
qqnorm(resid(m1))
```
...
__Case-based bootstrapped 95% CIs:__
...
Change in [dv] over [iv] was modelled using a linear mixed effects model, with a fixed effect of [iv] and a by-[pid] random intercepts. At baseline, scores on [dv] were estimated at `r res[1,2]` (case-based bootstrap 95% CI: `r res[1,3]`--`r res[1,4]`). Results indicated that, relative to time-point 1, scores at time-point 2 increased by `r res[2,2]` (`r res[2,3]`--`r res[2,4]`), and at time-point 3 had increased by `r res[3,2]` (`r res[3,3]`--`r res[3,4]`). 
