...
:::woo
https://app.wooclap.com/events/BLZCGEQ
:::


# Construct Validity {transition="slide"}
...
![](img_sandbox/biscuits/Slide4.PNG){.fragment}
:::
...
Some weird, archaic, but real examples:  

- "My father was a good man."   <<< Antisocial Tendencies
- "My hands and feet are usually warm enough."   <<< Anxiety
- "I would like to be a florist."   <<< Masculinity/Femininity  
- "I sometimes enjoy teasing animals".   <<<< Psychopathic Deviate
...
::::{.columns}
:::{.column width="70%"}
![](img_sandbox/biscuits/Slide11.PNG)
:::
...
\quad \\
&X\text{ is observed score on measure }X \\
&T_x\text{ is True score} \\
&e_x\text{ is error}
\end{align*}
$$
...
# How to Estimate Reliability {transition="slide"}
...
\qquad \\
\textrm{Where:}& \\ 
\sigma^2_{b} &= \textrm{variance between group} \\ 
\sigma^2_{e} &= \textrm{variance within groups (residual variance)} 
\end{align}
$$
...
<!-- ```{r} -->
<!-- #| echo: false -->
<!-- ddl = dd |> mutate(id = 1:n()) |> -->
<!--   pivot_longer(1:2,names_to="test",values_to="score") -->

<!-- pdl = ddl |> mutate(score=round(score,1)) |> -->
<!--   head() |> -->
<!--   rbind("...") |> -->
<!--   as.data.frame() -->
<!-- row.names(pdl)[7]<-"..." -->
...
- We have a sample of all possible items we might use for measuring X  
    - a set of exchangeable tests

## Opt 1: 'tau equivalent'
...
## Opt 1: ~~Cronbach's~~ Coefficient alpha
...
\sigma^2_X \text{ is the variance of the sum scores} \\
\sigma_{ij} \text{ is the covariance between items }i\text{ and }j \\
\end{align*}
$$
![](img_sandbox/angryequ.png){.fragment fragment-index=1  .print-only-hide style="position: absolute; top: 0; left: 0; width: 40%; z-index: 10;"}
:::
:::
...
## Opt 1: Assumed Measurement Model  
...
![](img_sandbox/ctt/Slide9.PNG){.fragment fragment-index=1}

:::

## Opt 2: McDonald's Omega
...
When we calculate a correlation using estimated quantities (e.g., a score from a set of items), they will be:  

- biased towards zero
- more precise than they should be  
...
$$
r^*_{xy} = \frac{r_{xy}}{\sqrt{ \rho^2_{xx'} \rho^2_{yy'}}}
$$
...
$\rho^2_{xx'}$ is the reliability of $x$  
$\rho^2_{yy'}$ is the reliability of $y$  

## what should we do?

At very least just _think_ about reliability! 

- report reliability estimates
- better yet, 'disattenuate' it  
- even better, include measurement error in the model itself (CFA, SEM, hierarchical modelling)  
...
$r_{xy}$ = 0.23, 95\% CI [0.03, 0.41]
<br>
$r*_{xy}$ = 0.76, 95\% CI [0.11, 1$^1$]
:::
::::

:::aside
$^1$ - censored at 1, because you can't have a correlation >1! 
:::


# Final thoughts {transition="slide"}
...
# EXTRA
if we have time and want some R coding.. 

## a task!  

![](img_sandbox/dotdens.png)

## some people saw: 

![](img_sandbox/dotdensscale1.png)

## some people saw: 

![](img_sandbox/dotdensscale2.png)
