...
\sigma^2_X \text{ is the variance of the sum score over all items in scale X} \\
...
# Pulling it together {transition="slide"}
...
any time you calculate correlation using estimated quantities (e.g., a calculated score from a set of items), consider the attenuation from unreliability! 

- at very least: report reliability estimates
- better yet, 'disattenuate' it?  
- even better, include measurement error in the model itself
...
# measurement is always developing {transition="slide"} {background-color="white"}  
