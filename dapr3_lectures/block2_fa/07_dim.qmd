...
- In psychology, education etc:  
    ...
    - often termed a "latent trait" or "ability" 
...
- exploring what underlying construct(s) best explain the pattern of responses to a set of questions
- testing theoretically pre-defined construct(s) in a confirmatory setting
...
- $Z()$ here is standardisation - $Z(x) = \frac{x - \bar x}{sd_x}$
...
downweighting Q3 means the error in that variable contributes less to the score!  
...
#| eval: false
#| echo: true
psych::principal(boxes[,-1], nfactors = 11, rotate="none")
```
:::
