...
# Warm up Activity {transition="slide"}

What is measurement?  

:::woo
https://app.wooclap.com/events/TLMAGFI
:::


# Measurement {transition="slide"}
...
- is someone scoring 50 twice as "satisfied with life" as someone scoring 25?  
...
3. **standardisation** to ensure numeric results are trustworthy and invariant across different objects, instruments, and observers; 
4. **mathematical analysis** to map numeric values to the target attribute and quantify uncertainty. 
...
![](img_sandbox/dougnuts/Slide1.PNG)
...
# An aside: Measurement in the modern world   {transition="slide"}
...
#| eval: false
#| echo: true
lsatdat$ScaleScore = rowMeans(lsatdat[,3:5])
```
:::
:::{.codewindow .r}
...
tibble(
  ...
  description = c(
    ...
    "The conditions of my life are excellent",
    "I feel content with how things have gone today",
    "Average of Scores on Q1, Q2 and Q3"
  )
) |> gt::gt(caption="Data Dictionary")
```
:::
...
- we then discard the components that don't capture as much variation 
...
![](img_sandbox/pca_firstlook/Slide1.png){width="1000px"}




# A bigger example {transition="slide"}
...
dff$Q4 = rnorm(100,rowMeans(cbind(df$Q1,.8*df$Q3)),15)
...
#| eval: false
#| echo: true
lsat_pca <- principal(lsat10[,2:11], nfactors = 10, rotate = "none")
...
# Using PCA in context {transition="slide"}
...
#| echo: true
#| eval: false
lsat_1pc <- principal(lsatdat[,3:5], 
          nfactors = 1, 
          rotate = "none") 
...
```{r}
#| eval: false
lsatdat$PC1 <- lsat_1pc$scores

head(lsatdat)
```
:::
```
    id RiskTake Q1 Q2 Q3 ScaleScore    PC1
...
<!-- #| eval: false -->
<!-- #| echo: true -->
<!-- lsat_pca2 <- principal(lsat10[,2:11], nfactors = 2, rotate = "none") -->
...
<!-- ```{r} -->
<!-- #| label: pca10scoresbind -->
<!-- #| eval: false -->
<!-- #| echo: true -->
<!-- cbind(data, pca$scores) -->
<!-- ``` -->
<!-- ::: -->
<!-- ```{r} -->
<!-- #| eval: true -->
...
<!-- ) |> select(id,Q1:Q10,PC1,PC2,RiskTake) -->
<!-- head(full_lsat) -->
<!-- ``` -->
...
#| label: mod3
#| eval: false
mod3 <- lm(RiskTake ~ PC1, data = lsatdat)
summary(mod3)
```
:::
...
boxes$tot_surf = boxes$area_f1*2 + boxes$area_f2*2 + boxes$area_f3*2
boxes$tot_vol = boxes$length_inches * boxes$width_inches * boxes$height_inches
  
names(boxes) <- gsub("_inches|onal","",names(boxes))
head(boxes)
```
...
#| eval: false
#| echo: true
principal(boxes[,-1], nfactors = 11, rotate="none")
```
:::
