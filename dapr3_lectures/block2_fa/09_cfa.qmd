...
- EFA is a **model** of underlying 'latent variables/factors' that give rise to responses on observed variables
    - the underlying factors are held to be real (unobservable) things
    - the whole idea is to figure out what they are
...
- diagrams more generally adhere to a system of formal logic
...
aim is to isolate flow of information from X to Y, along only the path(s) we are interested in.  
...
:::
:::{.column width="33%"}
path x -> z <- y is closed

controlling for z opens it
:::
::::


most common interpretation of "effect of X on Y": close the back-doors, keep causal front-doors open  
...
5. emphasise everything is just "correlations", so can't make any causal conclusions.  


## the grey area - "associations"

- it's not clear what people actually mean by "x is associated with y"  

is our question purely descriptive?  
"does X tend to appear with Y?"  
if so, we probably don't want to control for other things.  


is our question (implicitly) causal?  
"does X cause Y?"
"does X influence Y?"
"does X affect Y?"
"does X explain Y?"
...
"does X improve Y?"
"does X make Y?"
if so, we need to control for the things that allow us to get an unbiased estimate of what would happen to a units value of Y when X is changed.  

is our question "associational"?  
what does that mean? 
"X tends to appear with Y, when holding A,B,C constant". 
why A,B,C? why not also D? 
