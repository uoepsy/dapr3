...
p1 = ggplot(df, aes(x=x,y=y))+geom_point()+
  ...
  labs(title="Raw X")+
  scale_x_continuous(limits=c(0,max(df$x)),breaks=0:9)+
  themedapr3()
...
p2 = ggplot(df, aes(x=x,y=y))+geom_point()+
  ...
  scale_x_continuous(limits=c(0,max(df$x)),breaks=map_dbl(seq(4,-4), ~mean(df$x)-.),
                     labels=seq(-4,4))+
...
p4 = ggplot(df, aes(x=x,y=y))+geom_point()+
  ...
  geom_smooth(method="lm",fullrange=T,lty="dotted")+
  #ylim(0,max(df$y))+
  geom_segment(x=3,xend=3,y=0,yend=max(df$y))+
  geom_segment(x=0,xend=30,y=0,yend=0)+
  geom_point(data=tibble(x=3,y=coef(mod)[1]),size=4)+
  scale_x_continuous(limits=c(0,max(df$x)),
                      breaks=c(0,3,6,9),
                     labels=seq(0,9,by=3)-3)+
  labs(title="x-3")+
  themedapr3()
p1 + p2 + p3 + p4 
```
