...
But these don't help if have violated our assumption of independence...
...
- Completely partition out any variance due to clustering into fixed effects for each cluster (__no pooling__).
    - and limit our estimates to being cluster specific
...
sums squares can be calculated differently

sequential = order matters for unbalanced designs
...
partial = each one as if its the last one in sequential
...
#df %>% filter(sid !="a1") -> df
df %>% mutate(
  gg = as.numeric(factor(condition))+(as.numeric(gsub("a|b|c|d", "",sid))-5)/10
) %>%
  ggplot(.,aes(x=gg,y=y))+
  geom_point(aes(group=gg, col=sid), alpha=.4,size=5)+
  geom_boxplot(aes(group=condition, col=condition),alpha=.3)+
  guides(col=FALSE,fill=FALSE)+
  themedapr3()+
  scale_x_continuous("condition",breaks=1:4, labels=c("a","b","c","d"))
```
]
.pull-right[
```{r}
