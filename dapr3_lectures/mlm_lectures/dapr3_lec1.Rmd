...
tibble(
  ...
  mutate(
    nworms = round(7 - 3*birdx + 3.5*gardenmeanx -1*(birdt=="blackbird") + .5*(birdt=="blackbird")*birdx + rnorm(n(),0,2)),
    nworms = pmax(0,nworms),
    arrival_time = round(birdx,2)
  ) %>% select(gardenid, arrival_time, nworms, birdt) -> df

df <- df %>% mutate(x = arrival_time, y = nworms, x2 = birdt, birdid=1:n())

df <- df[-c(35:40),]
df$y[df$birdid==33]<-17
df$nworms[df$birdid==33]<-17
df[df$gardenid=="garden4",] %>% ggplot(.,aes(x=x,y=y))+geom_point()
...
ggplot(df,aes(x=arrival_time,y=nworms))+geom_point()+
  ...
  ggplot(df,aes(x=arrival_time,y=nworms,col=gardenid))+geom_point()+
...
```{r}
library(plm)
clm <- plm(y~x,data=df, model="pooling", index="gardenid")
...
```{r}
library(geepack)
df <- df %>% arrange(gardenid) %>%
  mutate(
    id = as.numeric(as.factor(gardenid))
  )
...
```{r}
library(lme4)
model <- lmer(nworms~arrival_time+(1|gardenid),df)
summary(model)$coefficients
```
...
```{r echo=FALSE}
dfm <- 
  df %>% group_by(gardenid) %>%
  summarise(arrival_time = mean(arrival_time)) %>% 
  mutate(mf = predict(model, newdata=.),
         ff = fixef(model)[1] + arrival_time*fixef(model)[2])
...
ggplot(., aes(x=arrival_time, y=nworms))+
  geom_line(aes(y=fit, group=gardenid, col=gardenid))+
  geom_abline(intercept=fixef(model)[1], slope=fixef(model)[2],lwd=2)+
  geom_point(aes(col=gardenid),alpha=.2)+
  geom_segment(aes(y=nworms, yend = fit, x = arrival_time, xend = arrival_time, 
                   col=gardenid), alpha=.2)+
  geom_segment(data=dfm, aes(x=arrival_time, xend=arrival_time, y=mf, yend=ff))+
  themedapr3()
```
