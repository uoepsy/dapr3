...
summary(eds_data$EDS)
summary(eds_data$CRQ)
table(eds_data$schoolid)
table(eds_data$schooltype)
...
# - one version with schooltype as colour
eds_data |>
  ggplot(aes(x = CRQ, y = EDS, colour = schooltype)) +
  geom_point()
# - another version with schooltype as facets
eds_data |>
  ggplot(aes(x = CRQ, y = EDS)) +
  geom_point() +
...
# - grouping variable (i.e., if each schoolid has multiple observations)
eds_data |>
  group_by(schoolid) |>
  count()
...
# - Just computed how many children per school (see above).
...
# - Does each level of schoolid appear with at least one value of CRQ? 
# - Yes, though the xtabs trick is a bit silly with a continuous predictor.
xtabs(~ schoolid + CRQ, data = eds_data)
# - A trick from the flash cards to count all the non-zero values per school: 
(xtabs(~ schoolid + CRQ, data = eds_data)  != 0) |> rowSums()

# Does each level of schoolid appear with at least one value of schooltype? 
# - No.
...
# - Some deviations at extremes.
# - This is because EDS is bounded between 1 and 6, and we see the taper that reflects this.
# - But this LMM assumes outcome is truly continuous.
# - Beyond DAPR3, there are other kinds of generalised LMs that are better for 
#   bounded data. But it's OK for now.
...
# - ^ better except for one slope adjustment being way smaller than we'd expect!
...
Effect(focal.predictors = c("CRQ", 'schooltype'), mod = eds_m1) |> 
  as.data.frame()

# - CRQ is spaced weird, so we'll ask for more levels
Effect(
  focal.predictors = c("CRQ", 'schooltype'), 
  mod = eds_m1, 
  xlevels = 20  # how many levels for continuous x axis predictor
) |>
  as.data.frame() 
...
#   suggests that 95% of the adjusted school-level intercepts fall between 
