...
tel_data |>
  ...
  stat_summary(geom = 'pointrange', fun.data = mean_se, size = 1, linewidth = 1.5) +
  labs(
...
The fixed effects part of the model formula will be `Correct ~ Delay * Group`.
...
The remaining variables: `Subject_ID` (categorical), `Test_word` (categorical), and `Rtime` (continuous, so we don't need to check it).
...
The minimum LMM formula now: `Correct ~ Delay * Group + (1 | Subject_ID) + (1 | Test_word)`
