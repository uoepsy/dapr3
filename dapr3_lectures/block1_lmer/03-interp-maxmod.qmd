...
:::dapr3callout
In the model summary for an LMM, how do we interpret the random effect summaries?
...
- By thinking about the data generating process, and specifically the kind of variability that the grouping variable contributes to the data.
...
- A model that contains all possible random effects that are permitted by the data structure.
:::
...
- The estimated mean log reaction time when the topic pairing is unassociated (the reference level, coded as 0) is 4.77 log units.
- (it's usually only interesting to talk about the intercept's p-value if we're fitting a logistic regression model, so for continuous outcomes like log RT, we will skip it)
...
**Then:** Look at the rest of the output and try to read it like a table like this:

![](figs/w3-ranef-tab.svg){fig-align="center"}


## Intercept adjustments for each `ppt_id`
...
## `pairingAssociated` adjustments for each `ppt_id`
...
**The `pairingAssociated` adjustments (i.e., slope adjustments over `pairing`) for each level of `ppt_id` have a SD of 0.31 log units.**
...
## `pairingAssociated` adjustments for each `item_id`
...
Effect(
  focal.predictors = c("pairing"),   # what predictor do we want predictions for?
