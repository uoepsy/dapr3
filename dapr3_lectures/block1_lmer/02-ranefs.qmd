...
**Example of an IAT to test whether you implicitly relate anxiety with yourself:**
...
![](figs/TODO-w2-IAT-stressed.jpg){width=50% fig-align="center"}
...
![](figs/TODO-w2-IAT-self.jpg){width=50% fig-align="center"}
:::
::::
:::
...
Reaction times in milliseconds are mostly very small, so **the assumption of normal distribution of errors is not met.**
...
`item_id`: the word that people categorise (e.g., "self", "supported", "calm").
...
```{r}
stats::xtabs(
  ~ ppt_id + item_id,    # write the variables to crosstabulate in this weird format
  data = implicit_data
)
```
...
## How do we tell a model that the relationship between variables can be different for different participants and different items?
...
## The first wrong way to model this data: <br> Just fit a simple LM

```{r}
implicit_lm <- lm(
  logRT ~ pairing,  # predicting logRT as a function of pairing (unassoc. vs. assoc.)
  data = implicit_data
)
```
...
- **estimates the average relationship across the whole dataset**
...
When we include `(1 | ppt_id)` in the model formula, we tell the model to **estimate how much the average intercept should be nudged up or down by,** in order to fit the data from each participant.

The model can give us a list of adjustments, one for each `ppt_id`, for example:
...
- The by-participant adjustments to the intercept (and how variable they are).
  - Also called **random intercepts** or **varying intercepts**.
...
- These are the same $\beta$ coefficients you got to know in DAPR2.
...
- The `1` represents the adjustment to the fixed intercept.
- The `+ pairing` represents the adjustment to the fixed slope over `pairing`.
...
> We fit a linear mixed model to predict log RT as a function of topic pairing (treatment-coded, with "Unassociated" coded as 0 and "Associated" coded as 1).
...
> ... This model includes random intercepts and random slopes over topic pairing, both by participant and by item.
...
Lots of options, so golden rule: **be extremely clear about which grouping variables get which intercept/slope adjustments.**
...
## Interpreting the model summary (1): <br> Fixed effects
