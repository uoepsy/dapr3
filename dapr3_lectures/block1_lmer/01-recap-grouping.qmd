...
## Part 3: Think back to the properties of lines
...
- To answer our question, we will first assume that in reality, the true slope of this line is zero: the groups are not different at all (= our null hypothesis).

- We know the distribution of values the slope might have in that reality where the groups are no different.

- We compare our observed slope to that null distribution.

- If the observed slope falls in either tail of that null distribution, then we can say that the slope is significantly different from zero!
...
**`(Intercept)`:** The estimated average `lifesat` for the reference level of `condition`, which is `journal`.

- People who journal are estimated to have a life satisfaction of 40.95 points, on average.

<br>

**`conditionmeditate`:** The difference between the average `lifesat` when `condition = meditate` and the average `lifesat` when `condition = journal`.
...
As soon as we have repeated-measures data, **the data points are no longer independent.**
...
- If we know one data point from `j16`, we can make some good guesses about what values `j16`'s other data points might have. Same for `m9`.
- In other words: Some of our data gives us information about other parts of our data. 
- In other words: Data points from one person are not independent of other data points from that person.
...
When we have multiple observations from a single source (here, a single person), there's a one more kind of variability that we need to account for.
