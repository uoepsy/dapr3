...
block1_lecs = c("linear models refresher",
                "working with groupings of data (the multilevel model)",
...
:::aside
I've stopped showing the $i$'s for now because we start to have more subscripts, but for any individual $i$, our model is $\color{red}{y_i} = \color{blue}{b_0 \cdot{} 1 + b_1 \cdot{} x_{1i} + \, ... \, + b_k \cdot x_{ki}} + \varepsilon_i$
:::
...
:::{.notes}
so accounting for Z differences can make it easier to detect differences due to x
