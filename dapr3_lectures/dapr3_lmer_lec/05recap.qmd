...
- Personally, I prefer plots, but if you must...   
...
mf |> 
  ...
  stat_summary(geom="line",aes(y=.fitted,group=ppt))+
  facet_wrap(~schoolid)
...
mf |> filter(uppt %in% sample(unique(mf$uppt),30)) |>
  ggplot(aes(x=ageC,col=interv)) +
  geom_line(aes(y=.fitted))+
  geom_point(aes(y=ABS))+
  facet_wrap(~interaction(ppt,schoolid))
...
- often it's about the amount of variability relative to the corresponding fixed effect.  
    - Consider: 
        ...
        - a fixed slope of 0.5 with groups varying in slopes with an SD of 0.01
...
modelsummary(absmod, 
             ...
             statistic = "({p.value})",
             fmt = fmt_decimal(2, 3),
             gof_omit = 'IC|RMSE|R2|AIC|BIC'
)
```

See [https://modelsummary.com/vignettes/get_started.html](https://modelsummary.com/vignettes/get_started.html){target="_blank"}
