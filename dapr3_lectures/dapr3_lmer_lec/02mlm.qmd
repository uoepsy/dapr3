...
- introduction to the multilevel/mixed effects model
    - how to think about it (model structure)
    - how to fit it (in R)
    
:::woo
https://app.wooclap.com/events/PQBDDT/votes?view=desktop&mobile=false
:::
...
# Building up the idea - in pictures
...
::::{.columns}
:::{.column width="50%"}
models group-level *variability*, rather than estimating group *differences*.  

think "a distribution of possible lines"  
...
## Checkpoint! {background-color="white"}

:::woo
https://app.wooclap.com/events/PQBDDT/votes?view=desktop&mobile=false
:::

# Building up the idea - in equations
...
& \color{blue}{b_{0i}} = \gamma_{00} + \color{orange}{\zeta_{0i}} \\
\quad \\
& \color{orange}{\zeta_{0i}} \sim N(0, \color{orange}{\sigma^2_0}) \\
...
& \quad \\
& \color{orange}{\zeta_{0i}} \sim N(0, \color{orange}{\sigma_0^2}) \\
...
& \text{for observation }j\\
\quad \\
& \color{red}{y_{j}} = \color{blue}{b_{0} \cdot 1 + b_{1} \cdot x_{j}} + \varepsilon_{j} \\
\quad \\
...
```{r}
#| echo: false
mod1 = lm(lifesat~I(age/10),data=d3)
basep + 
  geom_abline(intercept=coef(mod1)[1],slope=coef(mod1)[2], lwd=1,col="blue")
```

:::
::::

- Things that we observe: $\color{red}{y_j}$, $\color{blue}{x_j}$  
- Things the model actually estimates: $\color{blue}{b_0}$, $\color{blue}{b_1}$, $\sigma_\varepsilon^2$  
- Things that kind of 'fall out' of the model after estimation: $\varepsilon_j$ 
...
- Things that we observe: $\color{red}{y_{ij}}$, $\color{blue}{x_{ij}}$  
- Things the model actually estimates: $\color{blue}{b_1}$, $\gamma_{00}$, $\color{orange}{\sigma^2_0}$, $\sigma^2_\varepsilon$  
- Things that kind of 'fall out' of the model after estimation: $\color{orange}{\zeta_{0i}}$, $\varepsilon_{ij}$ 
...
- "random" because considered a random sample from larger population such that $\color{orange}{\zeta_{0i}} \sim N(0, \color{orange}{\sigma^2_0})$.  

:::{.fragment}

- No: Edinburgh just "randomly" happens to be higher than average
- Yes: Edinburgh just one of a "random sample" of dwellings. It is high in `lifesat` because of various reasons (it's a nice city, there's less rain than the west coast, etc.).   
...
- estimate a bunch of specific differences between dwellings?  
- estimate the variability between dwellings  
...
| Random^[If we only have a very small number of groups, estimating variance components may be unstable, and partialling out group-differences as fixed effects *may* be preferable.]<br>$y\,\sim\,...\,+\,(\,... |\,g)$ | Different groups would be used   | A population from which the groups used are just a (random) sample |
...
&nbsp;&nbsp;the deviation of group $i$ ($\color{orange}{\zeta_{1i}}$) from $\gamma_{10}$  
<br>
group deviations for intercepts and slopes are normally distributed with mean of 0 and standard deviations of $\color{orange}{\sigma^2_0}$ and $\color{orange}{\sigma^2_1}$ respectively, and with a correlation of $\color{orange}{\rho_{01}}$. 
...
## the multilevel model: partial pooling {visibility="uncounted"}
...
## the multilevel model: partial pooling  (2)

![](img_sandbox/output_both_init.gif){width=800px}

## the multilevel model: partial pooling (2) {visibility="uncounted"}

![](img_sandbox/output_both.gif){width=800px}

## the multilevel model: partial pooling (3)

![](img_sandbox/output_bothK.gif){width=800px}

## the multilevel model: partial pooling (4) 

![](img_sandbox/output_bothS.gif){width=800px}
...
- L2 = children (M/B)
- L1 = observations (lines)

:::

## MLMs for MLQs (2) {visibility="uncounted"}
...
# A Task! {background-color="white"}
  

[https://edin.ac/41xRFJC](https://edin.ac/41xRFJC){target="_blank"}

![](img_sandbox/qr_expt.png)
...
- in `lmer()`, we have to also specify the random effect structure in parentheses:  
    ...
    &emsp;&emsp;&emsp;&emsp;data = __*[name of dataframe]*__,   
    &emsp;&emsp;&emsp;REML = __*[TRUE/FALSE]*__
...
using the **{lmerTest}** package will overwrite the `lmer()` function, and give you hypothesis tests for the fixed effects:
...
The `anova()` function will perform a "likelihood ratio test" to allow you to compare two models: 
```{r}
#| echo: true
...
:::aside
Technical note: This is not an "ANOVA", this is a likelihood ratio test. It doesn't analyse variances ($\Sigma(y_i - \hat y)^2$), it compares likelihoods (i.e., $P(data|model)$)
:::



## Inference
### it's (almost always) about fixed effects


- vast majority of research questions are about **fixed effects**, not random effects

- model comparisons should differ in only fixed effects, and have the same random effects


```{r}
set.seed(753)
df <- junk::sim_basicrs(b0=5,b1=1)
df <- df |> filter(g %in% unique(df$g)[10:16])
m01 <- lmer(y~1+(1+x|g),df)
m11 <- lmer(y~1+x+(1+x|g),df)
m10 <- lmer(y~1+x+(1|g),df)
m00 <- lmer(y~1+(1|g),df)
...
plotf <- function(mod){
  broom.mixed::augment(mod) |>
    ...
    geom_line(aes(group=g),alpha=.6)+
    geom_abline(intercept = fixef(mod)[1],slope=fixef(mod)[2],
                col="blue",lwd=1)
}
p01 <- plotf(m01) + 
  geom_abline(intercept = fixef(m01)[1],slope=0,
                col="blue",lwd=1) +
  labs(title="lmer(y~1+(1+x|g))")
p11 <- plotf(m11) +
  labs(title="lmer(y~1+x+(1+x|g))")
p10 <- plotf(m10) +
  labs(title="lmer(y~1+x+(1|g))")
p00 <- df |> mutate(.fitted = predict(m00)) |>
  ggplot(aes(x=x,y=.fitted))+
    geom_point(aes(y=y),alpha=.2)+
    geom_line(aes(group=g),alpha=.6)+
    geom_abline(intercept = fixef(m00)[1],slope=0,
                col="blue",lwd=1) +
  labs(title="lmer(y~1+(1|g))")
  
(p00 + p01) / (p10 + p11) & theme_classic()
...
- testing random effects is difficult

- generally not necessary as we motivate their inclusion based on our understanding of the study design (and simplifications)
