...
we can use  as shorthand, but it is often preferable to separate them out (allowing different random effects for schools vs for children)
...
```{r}
#| echo: false
p1 <- ggplot(df,aes(x = 1, y = score, col = task)) +
...
p2 <- ggplot(df,aes(x = 1, y = score, col = task)) +
  geom_jitter(size=3,height=0,width=.2,alpha=.6)+ 
  facet_wrap(~task) +
  labs(title="tasks")+
  scale_x_continuous(limits=c(.5,1.5),breaks=NULL)+
