...
:::woo
https://app.wooclap.com/events/KEDOQR/votes?view=desktop&mobile=false
:::
...
+ Linear models "explain"^[this word is laden with causality, just because we can fit lm(y~x) doesn't mean that x in any way "explains" y] variance in an outcome.
...
## ![](img_sandbox/tangent.png){width=35px} Third variables (2)
...
## ![](img_sandbox/tangent.png){width=35px} Third variables (3)
...
## ![](img_sandbox/tangent.png){width=35px} Third variables (4)
...
## ![](img_sandbox/tangent.png){width=35px} Third variables (5)
...
mydata <- df |> mutate(drug = factor(drug, 
                                     levels=0:1,
                                     labels=c("No","Yes")))
...
broom::tidy(mod1) |>
  transmute(
    ...
    est=round(estimate,2),
    interpretation=c(
      "estimated BP when all predictors are zero (age 0, drug = no)",
      "estimated change in BP when age increases by 1, and drug is held constant",
      "estimated change in BP between drug = no and drug = yes, holding age constant"
    )
  ) |> gt::gt()
```
![](img_sandbox/caus/Slide6.PNG)
...
## ![](img_sandbox/tangent.png){width=35px} traditional ANOVA/ANCOVA (2)  {.smaller}
...
aim of making **models of the world** (models of how we think the data are generated).  
