...
- Basic rules about path diagram

---
# Estimation: ML
- Reiterate ML. 
- Make point that there are lots of different estimation methods but for this course ML is fine.
- Flag when to use others as a very simple set of rules.
- Flag this is a topic to go back to if they end up doign more complex analyses in dissy

---
# Path tracing 
- all based on covariances or correlations
- Can work out correlations from paths.
- This leads to an important concept
  - Our model represents the correlations
  - If our model contains less paths than we have correlations, then we are representing those relationships in a simplified form
- When we have less paths than correlations, our model is called identified. (positive df)
- When we have the same number it is called just identified (0 df)
- When we have more, our model is not identified and wont estimate (trying to do too much with data) (-ve df)

- When models are identified, we may not perfectly reproduce the correlations we observed in the data
  - That is the model implied covariance/correlations differ slightly from the actual data
  - The comparison of these two is at the heart of model fit (next section)
