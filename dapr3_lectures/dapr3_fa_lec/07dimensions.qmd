...
[INSERT QR CODE HERE FOR THE SURVEY!]  
...
#| include: false
set.seed(1182)
#eseed=round(runif(1,1,1e4))
#set.seed(eseed)
SS = matrix(runif(36,0.7,1), ncol = 6)
SS[1:5,6] <- SS[6,1:5] <- runif(5,.1,.3)

mystudy <- MASS::mvrnorm(n=75, mu=rep(0,6),Sigma = crossprod(SS)) |> as_tibble() |>
  mutate(across(everything(), ~as.numeric(cut(.,7))))
names(mystudy) <- paste0("Q",1:6)
mypca <- principal(mystudy, nfactors = 1, rotate = "none")  
mystudy <- mystudy |>
  mutate(
    BART = round(rnorm(n(),10+-2*mypca$scores+.9*Q6,3))
  )
# lm(BART ~ Q1 + Q2 + Q3 + Q4 + Q5 + Q6, data = mystudy) |> summary()
# library(modelsummary)
# modelsummary(
#   list(lm(mystudy$BART~mypca$scores[,1]),
#   lm(mystudy$BART~rowSums(mystudy[,1:6]))),
#   statistic="{p.value}"
# )
