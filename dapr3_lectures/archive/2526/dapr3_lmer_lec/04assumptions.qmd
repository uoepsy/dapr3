...
![](img_sandbox/bigfishlittlefish.png)
...
ggplot(bflp, aes(x=fish_weight, y=self_esteem, group=pond, col=pond))+
  geom_point(alpha=.7,size=3)+
  geom_segment(data=sdff,
               aes(x = fish_weight, xend = fish_weight, 
                   y = self_esteem-.1, yend = self_esteem+.1),
               lwd=3) +
  labs(x="Fish Weight (kg)",y="Self Esteem Scale (1-5)")+
  guides(color="none")
```
...
<center>**Within group**<br> __$x_{ij} - \bar{x}_i$__ </center><br>
```{r}
#| echo: false
bflp %>% group_by(pond) %>%
  mutate(
...
<center>**Between group**<br> __$\bar{x}_i$__ </center><br>
...
broom.mixed::augment(mod_wb) %>%
  ggplot(.,aes(x=fw_between_pond, y=self_esteem, group=pond))+
  geom_point() + #geom_line(alpha=.2) +
  geom_abline(intercept=fixef(mod_wb)[1], slope=fixef(mod_wb)[2]) -> p1

broom.mixed::augment(mod_wb) %>%
  ggplot(.,aes(x=fw_within_pond, y=self_esteem, group=pond))+
  geom_point() + #geom_line(alpha=.2) +
  geom_abline(intercept=fixef(mod_wb)[1]+(fixef(mod_wb)[2]*mean(bflp$fw_between_pond)), slope=fixef(mod_wb)[3]) -> p2

broom.mixed::augment(mod_wb) %>%
  ggplot(.,aes(x=fw_within_pond + fw_between_pond, y=.fitted, group=pond))+
