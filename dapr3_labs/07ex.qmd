...
`r qbegin(qcounter())`
We're finally getting somewhere! Let's create a score for "impulsivity".  
...
There's much more variability in the `pc1` values than the scale score values. This makes sense, because if the principal component scores are some weighting of each variable, then we're going to end up with far more possible scores.  
