...
It looks like most of these lines are going upwards, but there's a fair bit of variability in people's slopes.  

So we can actually make a guess at what we're going to see when we model. We'll probably have a positive fixed effect of `hunger_gc` (i.e. *A* below will be positive), and the by-participant variation in these slopes will be quite large relative to the fixed effect (i.e *B* below will be quite large in comparison to *A*)
...
```{r}
#| echo: false
p1 <- effect("avg_hunger*diet", hangrywbdiet, xlevels=20) |>
  as.data.frame() |>
  ggplot(aes(x=avg_hunger, y=fit,col=diet))+
  stat_summary(
    data = broom.mixed::augment(hangrywbdiet),
    aes(x = avg_hunger, y = .fitted),
...
predfr = hangryfull |> 
  distinct(ppt,diet,avg_hunger) |>
  mutate(hunger_gc = list(seq(-31,23,1))) |>
  unnest(hunger_gc)
predfr$.fitted = predict(hangrywbdiet,newdata = predfr)

p2 <- effect("hunger_gc*diet", hangrywbdiet, xlevels=20) |>
  as.data.frame() |>
  ggplot(aes(x=hunger_gc, y=fit,col=diet))+
  geom_line(
    data = predfr, 
    aes(x = hunger_gc, y = .fitted, group = ppt), alpha = .3) +
  geom_line()+
  geom_ribbon(aes(ymin=lower,ymax=upper,fill=diet),alpha=.4)+
  labs(x="increase from participants' average hunger level",
       y="estimated irritability")
