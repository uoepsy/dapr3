...
Recall the data from last week's exercises. Instead of looking at the roles A, B and C, we'll look in more fine grained detail at the seniority (the `rolespine` variable). This is mainly so that we have a continuous variable to work with as it makes this illustration easier.  

Below is some code that reads in the data, and fits a model of the workplace-pride predicted by seniority level. Line 5 then gets the 'fitted' values from the model and adds them as a new column to the dataset, called `pred_lm`. The fitte values are what the model predicts for every individual observation.  

Lines 7-10 then plot the data, split up by each department, and adds lines showing the model fitted values.  
...
library(tidyverse)
jsup = read_csv("../../data/lmm_jsup.csv")
...
Cope each chunk and run through the code. Pay attention to how the lines differ.  
...
Make sure to read about multilevel modesl and how to fit them in [2: MLM #fitting-multilevel-models-in-r](https://uoepsy.github.io/lmm/02_lmm.html#fitting-multilevel-models-in-r){target="_blank"}.  
...
You can see the effects package in [2: the multilevel model #a-more-complex-model](https://uoepsy.github.io/lmm/02_lmm.html?panelset_001=plot#a-more-complex-model){target="_blank"}.  
...
We've seen this last week too - counting the different levels in our data. There are lots of ways to do these things - functions like `count()`, `summary()`, `n_distinct()`, `table()`, `mean()`, `min()` etc. See [1: Clustered Data #determining-sample-sizes](https://uoepsy.github.io/lmm/01_clustered.html#determining-sample-sizes){target="_blank"}
