...
```{r eval=FALSE}
install.packages(c("tidyverse","ICC","lme4","lmerTest","pbkrTest","effects","broom.mixed","sjPlot","HLMdiag"))
...
ggplot(efdata, aes(x=PID, y=SDMT))+
  ...
  geom_hline(yintercept = mean(efdata$SDMT))+
...
```{r}
#| eval: false
lm(SDMT ~ audio * headphones, data = efdata)
```

However, we know that this simply ignores that our observations are not independent - we have repeated measures from the same individuals. Given that roughly 45% of the variation in SDMT scores can be attributed to variability between participants (as found when we calculated ICC), we don't want to mis-attribute this to differences between audio conditions.    

Using the "no pooling" method (i.e., including participant as a fixed effect), becomes difficult because the `PID` variable (the participant id variable) also uniquely identifies the two headphone conditions. Note that if we fit the following model, some coefficients are not defined.  
  
Try it and see:  
```{r eval=FALSE}
lm(SDMT ~ audio*headphones + PID, data = efdata)
```
