...
Before you estimate the model, how many degrees of freedom do you think the model will have?
Remember that degrees of freedom in CFA models is the the number of values that we know minus the number of parameters for the model to estimate (in short: knowns minus unknowns).
...
Think back to 'salient' being defined as $>|0.3|$ in EFA - these were standardised loadings (where everything - both latent variables and observed variables - is standardised).  
<!-- EP TODO: rephrase this bit -- is the threshold still 0.3 in CFA? -->
...
<!-- EP TODO: Write some prose that describes the take-aways from this output. -->
...
You can do this manually, which is probably worth it whenever you want to use these sort of diagrams in a proper write up, but there are some quick packages that do this in a rough-and-ready way. Check out the `semPaths()` function from the **semPlot** package [Chapter 5 CFA#making diagrams](https://uoepsy.github.io/lv/05_cfa.html#making-diagrams){target="_blank"}.  
...
The fit is still not great, but it's better (it was always going to be!), and the same suggested correlations are present in modification indices:  
```{r}
