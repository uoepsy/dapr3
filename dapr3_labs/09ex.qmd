...
- Reverse-code the questions with a negative direction—they'll just end up with loadings in the opposite direction, and that's fine!
...
```{r}
dasmod = "
Em =~ q1 + q6 + q10 + q11 + q17 + q19 + q21 + q23
...
What modifications do the model indices suggest? Are the top three suggestions theoretically reasonable, in your opinion?
...
`r qbegin(qcounter())`
Are all the theorised paths significant?  
Are the loadings all "big enough"?
Like in EFA, our reference value for "big enough" is $>|0.3|$.

::: {.callout-tip collapse="true"}
#### Hints

The easy way here is to just look at `summary()` of the model.  

Be aware, however, that depending on how you have scaled the latent variable, the significance tests might be more or less meaningful/interesting.
In the summary of lavaan models, the significance tests that are displayed are all with the null hypothesis that "the estimate is 0".
So the z-values (and so p-values) are different depending on the scaling. This is because with the marker method, the estimated factor loadings are "relative to the first loading", whereas with a standardised latent factor, they are all "with the factor variance set to 1".

Notice how in these two example models that are the same except for the scaling, the p-values are totally different:

```{r}
#| include: false
set.seed(975)
df = simulateData("F1=~.7*item1+.2*item2+.6*item3",200)
cfa("F1=~item1+item2+item3",df,std.lv=TRUE) |>
  summary()

cfa("F1=~item1+item2+item3",df) |>
  summary()
```
...
```{r}
#| eval: false
cfa("F1=~item1+item2+item3", somedata, std.lv=TRUE) |>
  summary()
```
```
...
Latent Variables:
  ...
  F1 =~                                               
    item1             0.850    0.285    2.985    0.003
    item2             0.158    0.070    2.264    0.024
    item3             0.519    0.179    2.906    0.004
```
...
```{r}
#| eval: false
cfa("F1=~item1+item2+item3", somedata) |>
  summary()
```
```
...
Latent Variables:
  ...
  F1 =~                                               
    item1             1.000                           
    item2             0.186    0.124    1.506    0.132
    item3             0.611    0.405    1.509    0.131
```
:::
::::
...
<!-- TODO: Write a sentence or two to describe the key take-aways from this output. -->
