...
One of the key things here is figuring out which direction the factor loadings are going. Depending on how many reverse-coded items we have loaded on to a factor, the directions could all just flip, because the latent variable doesn't know which direction it is looking - a latent variable on which being high = "more apathy" and low = "less apathy" is just a mirror image of one in which being high = "less apathy" and being low = "more apathy".  
...
`r qbegin(qcounter())`
Reverse code the items that are negatively directed towards apathy. 

We can see the data dictionary for which ones these are, but they are questions 2, 3, 4, 5, 7, 8, 9, 10, 13, 14, 16, 18, 20, and 22.  

::: {.callout-tip collapse="true"}
#### Hints

Remember we saw how to do this in [TODO](label){target="_blank"}
...
`r qbegin(qcounter())`
Refit the model now that you've reverse coded those questions.  
Challenge: what do you predict will happen? What will happen to the loadings? and to the factor correlations?  

`r qend()`
`r solbegin(show=params$SHOW, toggle=params$TOGGLE)`

My prediction here is now that for all the items, higher scores represent _more_ apathy, that means all the loadings will be positive, and the correlations between factors too. 
The magnitudes will be the same as before, just not negative!   
