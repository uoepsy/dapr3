...
There was considerable variability in baseline (i.e. no-audio) SDMT scores across participants (SD = `r res2[1,5]`), with participants also varying in the effects of music (SD = `r res2[2,5]`) and of white-noise (SD = `r res2[3,5]`). A weak negative correlation (`r res2[5,5]`) between participant-level intercepts and effects of white-noise indicated that people who score higher in the no-audio condition are affected more the presence of white-noise. A similar weak negative correlation (`r res2[6,5]`) between music and white-noise effects suggests participants who were more affected by one type of audio tended to be also be more affected by the other.
