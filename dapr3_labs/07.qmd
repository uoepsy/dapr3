...
1. Give it a clear name (for example, `dapr3_lab07.Rmd`).
...
Our research question: **is greater impulsivity associated with bigger losses when comparing online gamblers to casino gamblers?**    
...
**Task:** Convert them all to numbers, which we can then work with.  

::: {.callout-tip collapse="true"}
#### Hints

The code to achieve this is quite a lot, so we'll give you the structure that will work.  

🗂️ For a more in depth explanation of this and lots of other common data wrangling tasks for questionnaire data, see the [Questionnaire Data Wrangling](https://uoepsy.github.io/dapr3/2627/flashcards/block2/res_qdatawrangle.html) flash card.

In this case, we want to recode from words to numbers, and we want to do that operation *across* lots of variables.  
In the code below, wou'll want to change: 

- lines 1 and 2 to use the name of your dataset.  
- line 4 needs to specify the set of variables you want to do the operation to. it currently says "all the variables from `variable1` to `variablex` (and everything in between)".  
- lines 6-11 need to give the current values and what you want them to change to. Currently it will turn any entry of "?" to a 1, and any entry of "??" to a 2, and so on.  
...
`r qbegin(qcounter())`
Just looking at the impulsivity questions, create a correlation matrix of the 6 variables.  
What do you notice? Does it make sense given the wording of the questions?   

::: {.callout-tip collapse="true"}
#### Hints

To get a correlation matrix, we just select the columns we want, and then pass it to `cor()` function:  
```{r}
#| eval: false
data |>
  select(variable1:variablex) |>
  cor()
```

We can even visualise this by passing it to something like `heatmap()`:
```{r}
#| eval: false
data |>
  select(variable1:variablex) |>
...
🗂️
See the [Refresher: Variance, Covariance, Correlation](https://uoepsy.github.io/dapr3/2627/flashcards/block2/c00_covcor.html) flash card for how to read a correlation matrix. 
...
`r qbegin(qcounter())`
Reverse score questions 5 and 6, and then examine how the correlation matrix has changed.  


:::dapr3green
🗂️
See the [Questionnaire Data Wrangling](https://uoepsy.github.io/dapr3/2627/flashcards/block2/res_qdatawrangle.html#reverse-coding) flash card for how to reverse score variables. 
...
`r qbegin(qcounter())`
Create a score for "impulsivity" and add it as a new column onto the existing data frame.
...
:::dapr3green
🗂️
See the [How to: Calculate Scale Scores](https://uoepsy.github.io/dapr3/2627/flashcards/block2/p01_scalescores.html) flash card for how to calculate scale scores in R.   
:::
...
`r qbegin(qcounter())`
Is greater impulsivity associated with bigger losses when comparing online gamblers to casino gamblers?  
...
Based on the plot you make, what do you predict the coefficients from the model below will look like (where `impulsivity` is the scale score). Will the sign of each coefficient be positive or negative?  
```
lm(gain ~ impulsivity * online)
```
...
:::dapr3green
🗂️
See the [How to: Get PCA Scores](https://uoepsy.github.io/dapr3/2627/flashcards/block2/p01_pca.html) flash card for all things conducting PCA
:::
...
:::dapr3green
🗂️
See the [How to: Decide on the Number of Components/Factors](https://uoepsy.github.io/dapr3/2627/flashcards/block2/p01__pca_ndim.html) flash card.  
:::

`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`

Our research question, remember is asking "is greater impulsivity associated with bigger losses when comparing online gamblers to casino gamblers?"  
...
:::dapr3green
🗂️
See the [How to: Get PCA Scores](https://uoepsy.github.io/dapr3/2627/flashcards/block2/p01_pca.html) flash card for all things conducting PCA and extracting scores.  
:::
...
#| echo: false
print(
  principal(gdat[,2:7], nfactors = 1, rotate = "none")$loadings,
  sort=TRUE
)
```
...
`r qbegin(qcounter())`
Using your PCA scores, not the old summed scale scores, create a plot that shows the association between impulsivity and financial loss or gain in the two different locations (casino and online).
...
What changes, compared to the previous model that used the summed scale scores?
...
This makes sense, because if the principal component scores are differentially weighting each variable, then we're going to end up with far more possible scores.  
