...
`r qbegin("A7 - Harder")`
Can you now map those same coloured sections in @fig-lmersummap to the mathematical terms in the model equation:  
...
`r qbegin("A9")`
Plot the model fitted values but *only* for the people from "Glasgow City" and "Na h-Eileanan Siar" (Western Isles), and add the observed wellbeing scores too.  
Do this for both the model with the random intercept only, and the model with both the random intercept and slope.  
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
```{r}
augment(ri_model) %>%
  filter(str_detect(laa, "Glasgow|Eileanan")) %>%
  ggplot(aes(x = outdoor_time, y = .fitted, col = laa)) + 
  geom_line() + 
  geom_point(aes(y=wellbeing), alpha=.4)
...
It is available, in **.rda** format, at https://uoepsy.github.io/data/wellbeingwork3.rda 
:::

`r qbegin("C1")`
Load the data, and take a look at what is in there. Hopefully it should match the description above.  

**Hint:** `load(url("https://uoepsy.github.io/data/wellbeingwork3.rda"))`
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
```{r}
load(url("https://uoepsy.github.io/data/wellbeingwork3.rda"))
summary(wellbeingwork3)
head(wellbeingwork3)
```
`r solend()`
...
Parametric Bootstrap Likelihood Ratio test found that the inclusion of TimePoint significantly improved model fit over the null model ( $\chi^2(1)$ = `r round(res0$test$stat[2],2)`, `r ifelse(res0$test$p.value[2]<.001, "p < .001", paste0("p = ",round(res0$test$p.value[2], 3)))`), suggesting that, on average, participants' wellbeing changed over course of 36 month assessment period. 
...
> Q: Did the experimental condition groups differ in the how wellbeing changed over the assessment period?   
...
:::int 
Conditions differed overall in wellbieng change (bootstrap likelihood ratio = 11.38, $p = .003$ ).  
Conditions differed in change over assessment period (bootstrap likelihood ratio = 23.70, $p = .001$ ).  
:::
...
From your model from the previous question which investigates whether conditions differed in their rate of wellbeing change, can you state how the conditions differed?  
