...
```{r}
ranef(mod)$joke_id |>
  filter(`(Intercept)` == max(`(Intercept)`))
```
...
```
Joke 28 has the most benefit of video. The joke itself is a bit weird.. maybe the video really helped!  
```{r}
laughs |> count(joke_id, joke_label) |>
  filter(joke_id==28) |>
  pull(joke_label)
...
Next I removed the correlations between all random effects. This is not a model that I want to end up with, but it sometimes helps to make clearer to us which random effect terms are not contributing as much as others. 
```{r}
#| message: true
mod2 <- lmer(Rtime ~ Delay*Group +
             (1 + Delay || Subject_ID) +
             (1 + Delay + Group || Test_word),
             data=tel)
VarCorr(mod2)
...
You might note that we still do have random effect correlations in `mod2`, despite us using `||` to indicate that we didn't want to estimate them! This is because the `||` syntax removes correlations between the random effect terms, but not _within_ the levels of a categorical term! 

In this case, it looks like `Delay|Test_word` is actually capturing fairly little variance in our outcome in comparison to the other terms.  

So let's put back our correlations but remove `Delay|Test_word`:
```{r}
mod3 <- lmer(Rtime ~ Delay*Group +
             (1 + Delay | Subject_ID) +
             (1 + Group | Test_word),
             data=tel)
VarCorr(mod3)
```
We get no warning messages! Hooray! But we **do** see a correlation being estimated as -1.00. Let's perform the extra check for singular-fit:
```{r}
isSingular(mod2)
```
All looks good there. 

We could do a couple of things here for robustness checks. For instance, in `mod3a` below I remove Group|Test_word (which is the one with the correlation of -1).  In `mod3b`, I move `Group|Test_word` on to the right hand side of the random effects (thereby removing the correlation, see [Chapter 8 #simplifying-random-effect-structures](08_modelbuilding.html#simplifying-random-effect-structures){target="_blank"}.  

```{r}
# remove Group|Test_word
mod3a <- lmer(Rtime ~ Delay*Group +
             (1 + Delay | Subject_ID) +
             (1 | Test_word), 
             data=tel)
# move Group|Test_word onto the RHS
mod3b <- lmer(Rtime ~ Delay*Group +
             (1 + Delay | Subject_ID) +
             (1 | Test_word) + 
             (1 | Test_word:Group),
             data=tel)
...
The reason for this is to just check how estimates of fixed effects and their standard errors differ across these models. More often than not, this simply provides us with reassurance that the removal of random effects hasn't actually had too much of an impact on anything we're going to conduct inferences on. If they differ a lot, then we have a lot more to discuss!  
...
bind_rows(
 ...
 broom.mixed::tidy(mod3a) |> filter(effect=="fixed") |> mutate(mod="mod3a"), 
 broom.mixed::tidy(mod3b) |> filter(effect=="fixed") |> mutate(mod="mod3b")
) |> transmute(mod,term,
...
I would probably settle on `mod3` (with an explicit note about the correlation of -1) or `mod3b` (the structure of which is a bit more difficult/complicated to explain). Either way I would want to mention that `mod3a` (removal of `Group|Test_word` altogether) finds the same pattern of results.
