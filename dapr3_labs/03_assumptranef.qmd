...
Then we want to show the reaction times for each group, at each test (`min` or `week` delay).  
Do we want to show _all_ the datapoints, or maybe just the average for each group at each test (in which case, does `stat_summary()` help?).  
