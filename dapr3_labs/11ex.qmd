...
#| echo: false
#| out-width: "60%"
knitr::include_graphics("images/mock/efa2.png")
```
...
a. Interpret the fixed effects **[4 marks]**
b. Interpret the random effects **[4 marks]**
c. Discuss the relevance of the findings, considering the context of the study design and researchers’ aims **[2 marks]**
...
a. Interpret the fixed effects  

    - anger for someone who doesn't journal, at start (in "week 1", or "week 0" is fine here, give benefit of doubt) is `r res[1,4]`
    - no significant change over the study period for those who don't journal - 1 week is associated with a non-sig `r res[2,4]` change in anger
    - people who take the journal have significantly lower anger _at the start_ by `r res[3,4]`
    - change in anger for every week is significantly different for the journal group compared to the no-journal group. the weekly change is `r res[4,4]` lower for the journal group than the no-journal group
        - estimated weekly change for the journal group is $`r res[2,4]`+`r res[4,4]`=`r sum(res[c(2,4),4])`$

b. Interpret the random effects  
    ...
    - high level of ppt variability relative to fixed slope.  
        - we'd expect some ppts (even in the journal group) to _increase_ in anger
...
c. Discuss the relevance of the findings, considering the context of the study design and researchers’ aims  
    ...
    - But over 10 weeks they only go down by `r sum(res[c(2,4),4])*10`. still not clinically meaningful, but if change continues linearly beyond the study period, then this would translate to meaningful change after ~15 weeks
...
anything that makes sense here, more common ones are:
