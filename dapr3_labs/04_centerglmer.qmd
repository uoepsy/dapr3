...
`r qbegin(2)`
Based on previous questions, try to write out (in `lmer` syntax) an appropriate model to test the research aims.  
Try to think about the _maximal_ random effect structure (i.e. everything that _can_ vary by-grouping is estimated as doing so).  
e.g.:  
```{}
...
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
Overall, we are interested in the effect of hunger on irritability, and whether this effect is different for the five-two diet. 
