...
Sometimes, it may be useful to calculate the amount of variance in each item by its corresponding factor. 
These are just the standardised factor loadings (the estimates where *everything* is standardised), squared! 
...
standardizedsolution(myfittedmodel) |>
  ...
  select(lhs, rhs, est.std, r2)
```
What can then be useful is calculating the "average variance explained". This is kind of similar to the "variance explained" metrics we get out of EFA - for each factor, we sum the squared loadings and divide by the number of items! 
...
## Drawing your fitted model

To make proper, publishable diagrams of models, we would want to use different software like powerpoint, visio, inkscape etc. There are also tools online like <https://semdiag.psychstat.org/>. 

For just quick visuals though, R can do that! 
The **semPlot** package has the `semPaths()` function:  

```{r}
library(semPlot)
semPaths(object = myfittedmodel,
         # let's print the standardised estimates: 
         whatLabels = "std"
)
```
