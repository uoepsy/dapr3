...
5. How do we represent a person's standing on the dimensions?
    - Add up all the responses on each variable $\text{sum score} = y_1 + y_2 + y_3 +\, ... \, + y_p$, and if you want a mean-score, we then divide by the number of variables: $\text{mean score} = \frac{y_1 + y_2 + y_3 +\, ... \, + y_p}{p}$
...
5. How do we represent a person's standing on the dimensions?
    - Extract PCA scores (these are a weighted sum of the scores on each variables: $\text{score}_{\text{component j}} = w_{1j}y_1 + w_{2j}y_2 + w_{3j}y_3 +\, ... \, + w_{pj}y_p$)
...
5. How do we represent a person's standing on the dimensions?
    ...
    - In practice, CFA naturally extends to more advanced methods that allow us to estimate things of interest (e.g., relationships between constructs) _without_ having to ever estimate individual scores on the dimensions. Rather than a 2 step process of 1) estimate scores, 2) use scores in subsequent analysis, methods such as Structural Equation Modelling allow us to do it all in one.