...
- **Their relationships:** If there are several such grouping variables, how do they relate to each other?
  ...
  - Or if we collect observational data from many children across different schools, then our grouping variables are `child` and `school`. And because each child only attends one school, we can say that `child` is **"nested"** within `school`. (For how to model nested grouping variables, see the "Code for modelling nested grouping variables" rabbit hole in the [Add random effects to a model formula](ranef-formula.html) flash card).
...
- **Summaries of relevant variables by group**  (if included in the dataset):
...
### Summary stats for: How many observations per level?

Start by computing how many observations we have per participant (you'll have used this code before to determine whether `PID` is a grouping variable at all).
Then use `ungroup()` to remove the grouping by `PID`, so that we can compute some overall summary stats for the data in column `n`.
...
For `PID`, we have exactly 20 observations for each participant, with no variation, so this isn't so interesting to report.
If there *is* variation, then these numbers will be a bit more variable.
But note that not all of these summary stats will always be interesting or useful to report—use your judgement about what helps you tell your story.
